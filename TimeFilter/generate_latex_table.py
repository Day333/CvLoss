...
df = pd.read_csv('weather_timefilter_seeds_results.csv', keep_default_na=False)
...
def fmt(m, s):
    if pd.isna(m) or pd.isna(s): return "himBHs$"
...
def get_val(pl, loss_type, metric):
    return res.get(pl, {}).get(loss_type, {}).get(metric, 'himBHs_{\pm -}$')

def get_avg(loss_type, metric):
    return avg_res.get(loss_type, {}).get(metric, 'himBHs_{\pm -}$')
