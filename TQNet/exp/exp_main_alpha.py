...
class Exp_Main(Exp_Basic):
    def __init__(self, args):
        ...
        self.log_var_tmp = nn.Parameter(torch.zeros(1).to(self.device))
        self.log_var_add = nn.Parameter(torch.zeros(1).to(self.device))
    ...
    def _select_optimizer(self):
        model_optim = optim.Adam([
            ...
            {'params': [self.log_var_tmp, self.log_var_add], 'lr': self.args.learning_rate}
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_cycle) in enumerate(train_loader):
                ...
                if self.args.add_loss == "scv":    # spatial cross-variable
                    ...
                    #         loss_add = ((pred_diff - true_diff) ** 2).mean()
                    ...
                    #         pred_diff_chunk = out_nodes[:, chunk_i] - out_nodes[:, chunk_j]   # [B, chunk, L]
                    #         true_diff_chunk = y_nodes[:, chunk_i]   - y_nodes[:, chunk_j]     # [B, chunk, L]

                    #         diff_err = pred_diff_chunk - true_diff_chunk                      # [B, chunk, L]

                    #         pair_scale = true_diff_chunk.pow(2).mean(dim=(0, 2), keepdim=True).clamp_min(1e-6)
                    #         chunk_loss = (diff_err.pow(2) / pair_scale).mean()

                    #         loss_add += chunk_loss * (end - start)

                    #     loss_add = loss_add / num_valid_pairs
                    # else:
                    #     loss_add = torch.tensor(0.0, device=out_nodes.device)
                    ...
                    if num_valid_pairs > 0:
                        for start in range(0, num_valid_pairs, chunk_size):
                            ...
                            loss_add += (pred_diff_chunk - true_diff_chunk).pow(2).sum()
                ...
                if self.args.add_loss == "None":
                    ...
                    precision_tmp = torch.exp(-self.log_var_tmp)
                    precision_add = torch.exp(-self.log_var_add)
                    
                    loss = 0.5 * precision_tmp * loss_tmp + 0.5 * self.log_var_tmp + \
                           0.5 * precision_add * loss_add + 0.5 * self.log_var_add
                    
                    epoch_alphas.append((precision_tmp.item(), precision_add.item()))
                ##################### add #####################
            ...
            if len(epoch_alphas) > 0:
                avg_alphas = np.mean(epoch_alphas, axis=0)
                print(f"\tLearned Weights Average - Precision_MSE: {avg_alphas[0]:.6f}, Precision_Add: {avg_alphas[1]:.6f}")
                avg_add_list.append(avg_alphas[1])
                avg_tmp_list.append(avg_alphas[0])
        ...
        final_weight_tmp = torch.exp(-self.log_var_tmp).item()
        final_weight_add = torch.exp(-self.log_var_add).item()
        print("\n" + "="*50)
        print("Training Finished!")
        print(f"Final Weight for MSE (L_tmp): {final_weight_tmp:.6f}")
        print(f"Final Weight for Add Loss (L_add): {final_weight_add:.6f}")
        print(f"Tmp weight is {np.mean(avg_tmp_list)}")
        print(f"Add weight is {np.mean(avg_add_list)}")
        print("="*50 + "\n")
